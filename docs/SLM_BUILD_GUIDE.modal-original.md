...
corpus, this device, then deployed to a **website for inference**.
...
```bash
HUGGINGFACE_TOKEN=...  # a HF token with write access (for pushing the model)
```

The agent will `source .env.local` so that the trainiing can read HuggingFace. All three datasets are **ungated**,
...
- **A short markdown doc before each phase.** Before executing a phase, write a
  readable `docs/NN-<phase>.md` explaining what will run and why. 
